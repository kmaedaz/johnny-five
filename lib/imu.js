...
// https://www.pololu.com/product/2724
  LPS25H: {
    ADDRESSES: {
      value: [0x5c],
    },
    ...
    initialize: {
      value: function(board, opts) {
        var io = board.io;
        var address = opts.address || this.ADDRESSES[0];
        var freq = opts.freq || this.OPTS[0];
        opts.address = address;
        var computed = {
          pressure: null,
          temperature: null,
        ...
        io.i2cConfig(opts);
        //
        io.i2cWriteReg(address, this.REGISTER.CTRL_REG1, 0x90);
        ...
        var tmp_l;
          ...
          var readCycle = function() {
                setTimeout(function() {
                    io.i2cReadOnce(address, this.REGISTER.WHO_AM_I, 1, function(data) {
                    //              console.log("=+ " +data);
                     });


                    io.i2cReadOnce(address, this.REGISTER.TEMP_OUT_L, 1, function(data) {
                    tmp_l = data;
                    });
                    io.i2cReadOnce(address, this.REGISTER.TEMP_OUT_H, 1, function(data) {
                    tmp_h = data;
                    });
                    var tmp  =  tmp_h <<8   | tmp_l      ;
                    if( tmp & 0x8000 ){
                        tmp = -((tmp - 1) ^ 0xffff);
                    }
                    var  tmp1  =  42.5 + (tmp / 480.0)  ;
                    computed.temperature = tmp1 ;

                    io.i2cReadOnce(address, this.REGISTER.PRESS_POUT_XL, 1, function(data) {
                    press_xl = data;
                    });
                    io.i2cReadOnce(address, this.REGISTER.PRESS_POUT_L, 1, function(data) {
                    press_l = data;
                    });
                    io.i2cReadOnce(address, this.REGISTER.PRESS_POUT_HL, 1, function(data) {
                    press_hl = data;
                   var pressure =Fn.s32( Fn.s32(press_xl<<16)  |  Fn.s32(press_l<<8)  |  Fn.s32(press_hl) ); 
                    var tmp = (pressure / 4096);
                    computed.pressure = tmp / 10 ;
                    this.emit("data", computed);
                    readCycle();
                    }.bind(this));
    ...
    identifier: {
      value: function(opts) {
        var address = opts.address || Drivers.LPS25H.ADDRESSES.value[0];
        return "LPS25H-" + address;
      }
    },
  },
